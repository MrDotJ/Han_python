...
def Complementary_soc111(left_coeff, left_var, right_coeff, right_var, model, dual_expression, dual_obj, dual_var_name,
                        u, original_var_min, original_var_max, dual_var_min, dual_var_max, nnn):
    ...
    # # =========================== START =====================\
    # 添加三个辅助变量
    k1 = model.addVar(lb=-1 * gurobi.GRB.INFINITY, ub=gurobi.GRB.INFINITY)
    k2 = model.addVar(lb=-1 * gurobi.GRB.INFINITY, ub=gurobi.GRB.INFINITY)
    k3 = model.addVar(lb=-1 * gurobi.GRB.INFINITY, ub=gurobi.GRB.INFINITY)
    k_three = [k1, k2, k3]
    dual = [dual_left[0], dual_left[1], dual_right[0]]
    coeff = [left_coeff[0], left_coeff[1], right_coeff[0]]
    delta = []
    for ind in range(3):
        delta_k = (original_var_max[ind] - original_var_min[ind]) / (pow(2, nnn))
        delta.append(delta_k)

    for ind in range(3):
        # 每个辅助变量配NNN个二元变量
        v = model.addVars(nnn, vtype=gurobi.GRB.BINARY)
        for k in range(nnn):
            # dual_lower[ind] * u[ind][k] <= v[k] <= dual_upper[ind] * u[ind][k]
            # dual_lower[ind] * (1 - u[ind][k]) <= dual[ind] - v[k] <= dual_upper[ind] * (1 - u[ind][k])
            model.addConstr(dual_var_min[ind] * u[ind][k] <= v[k])
            model.addConstr(v[k] <= dual_var_max[ind] * u[ind][k])
            model.addConstr(dual_var_min[ind] * (1 - u[ind][k]) <= (dual[ind] - v[k]))
            model.addConstr((dual[ind] - v[k]) <= dual_var_max[ind] * (1 - u[ind][k]))
        model.addConstr(k_three[ind] == original_var_min[ind] * dual[ind] + sum([pow(2, n) * delta[ind] * v[n] for n in range(nnn)]))

    lagrange_sum_equal = gurobi.quicksum([coeff[i] * coeff[i] * k_three[i] for i in range(3)])
    model.addConstr(lagrange_sum_equal == 0)
    # # =========================== END =======================
...
def Complementary_soc(left_coeff, left_var, right_coeff, right_var, model, dual_expression, dual_obj, dual_var_name,
    ...
    for i in range(left_var_length + right_var_length):
        ...
        measurement.append([xi, xj, xi*xj, wij, wij - xi * xj, binary, xjn_min, xjn_max])
