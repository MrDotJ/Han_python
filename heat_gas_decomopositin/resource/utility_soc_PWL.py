...
def Complementary_great(expr, model1, model2, dual_expression, dual_obj, dual_var_name):  # expr should be greater than zero
    assert (type(expr) == gurobi.LinExpr or type(expr) == gurobi.Var)
    # 给 model2 添加 对偶变量
    var_dual = model2.addVar(name=dual_var_name)
    # 给 model1 添加 原约束
    model1.addConstr(expr >= 0, name='original_feasible_' + dual_var_name)
    # 给 model2 添加 对偶约束
    model2.addConstr(var_dual >= 0, name='dual_feasible_' + dual_var_name)
    return var_dual, [-1 * expr, var_dual]

def Complementary_equal(expr, model1, model2, dual_expression, dual_obj, dual_var_name):
    assert (type(expr) == gurobi.LinExpr or type(expr) == gurobi.Var)
    # 给 model2 添加 对偶变量
    var_dual = model2.addVar(lb=-1 * INFINITY, ub=INFINITY, name='dual_' + dual_var_name)
    # 给 model1 添加 原约束
    model1.addConstr(-1 * expr == 0, name=dual_var_name + '[EqualFeasible]')
    # 返回一个 list, 我要用这个 list 来 构建 一个新的表达式  ==> 全局约束 即 互补条件
    return var_dual, [-1 * expr , var_dual]

def Complementary_soc111(left_coeff, left_var, right_coeff, right_var, model, dual_expression, dual_obj, dual_var_name,
...
def Complementary_soc(left_coeff, left_var, right_coeff, right_var, model1, model2, dual_expression, dual_obj, dual_var_name,
                       original_var_min, original_var_max, dual_var_min, dual_var_max):
    left_var_length = len(left_coeff)
    right_var_length = len(right_coeff)
    # 左侧对偶变量
    dual_left = model2.addVars(left_var_length, lb=-1*INFINITY, ub=INFINITY,
                              name=dual_var_name + '_dual_left')
    # 右侧对偶变量
    dual_right = model2.addVars(right_var_length, lb=0, ub=INFINITY,
                               name=dual_var_name + '_dual_right')
    # 添加原锥约束
    expr_left = gurobi.quicksum([left_coeff[i] * left_coeff[i] * left_var[i] * left_var[i]
                                 for i in range(left_var_length)])
    expr_right = gurobi.quicksum([right_coeff[i] * right_coeff[i] * right_var[i] * right_var[i]
                                  for i in range(right_var_length)])
    model1.addConstr(lhs=expr_left, sense=gurobi.GRB.LESS_EQUAL, rhs=expr_right, name=dual_var_name + '[Original]')
    # 添加对偶锥约束
    dual_expr_left = gurobi.quicksum([left_coeff[i] * left_coeff[i] * dual_left[i] * dual_left[i]
                                      for i in range(left_var_length)])
    dual_expr_right = gurobi.quicksum([right_coeff[i] * right_coeff[i] * dual_right[i] * dual_right[i]
                                       for i in range(right_var_length)])
    model2.addConstr(lhs=dual_expr_left, sense=gurobi.GRB.LESS_EQUAL, rhs=dual_expr_right, name=dual_var_name + '[Dual]')
    # 添加互补约束
    lagrange_sum = gurobi.quicksum([left_coeff[i] * left_coeff[i] * left_var[i] * dual_left[i]
                                    for i in range(left_var_length)]) + \
                   gurobi.quicksum([right_coeff[i] * right_coeff[i] * right_var[i] * dual_right[i]
    ...
    dual_expression.append(-1 * lagrange_sum)
    dual_obj.append(0)
    return dual_left, dual_right, [[left_var[0], left_var[1], right_var], [dual_left[0], dual_left[1], dual_right] ]
...
def Complementary_soc_plus111(left_coeff, left_var, right_coeff, right_var, model, dual_expression, dual_obj, dual_var_name,
                           original_var_min, original_var_max, dual_var_min, dual_var_max):
    left_var_length = len(left_coeff)
    right_var_length = len(right_coeff)
    # 左侧对偶变量
