...
class OneLayer:
    ...
    def build_gas_system__(self):


        self.upper_well_quoted_price_tuple_dict   = \
            self.p2.addVars(self.well_upper_num, T, name='upper_gas_quoted_price')
        self.upper_well_quoted_price              = tonp( self.upper_well_quoted_price_tuple_dict)
        ...
        # # p1 variable
        self.upper_gas_well_output                = \
            tonp( self.p1.addVars(self.well_upper_num,  T, K, name='upper_well_output',     lb=-1 * INF, ub=INF ) )
        self.lower_gas_well_output                = \
            tonp( self.p1.addVars(self.well_lower_num,  T, K, name='lower_well_output',     lb=-1 * INF, ub=INF ) )
        self.gas_node_pressure                    = \
            tonp( self.p1.addVars(self.gas_node_num,    T, K, name='gas_node_pressure',     lb=-1 * INF, ub=INF ) )
        self.gas_flow_in                          = \
            tonp( self.p1.addVars(self.gas_line_num,    T, K, name='gas_flow_in',           lb=-1 * INF, ub=INF ) )
        self.gas_flow_out                         = \
            tonp( self.p1.addVars(self.gas_line_num,    T, K, name='gas_flow_out',          lb=-1 * INF, ub=INF ) )
        self.aux_weymouth_left                    = \
            tonp( self.p1.addVars(self.gas_line_num,    T, K, name='weymouth_left_aux',     lb=-1 * INF, ub=INF ) )
        self.aux_weymouth_right_1                 = \
            tonp( self.p1.addVars(self.gas_line_num,    T, K, name='weymouth_right_aux1',   lb=-1 * INF, ub=INF ) )
        self.aux_weymouth_right_2                 = \
            tonp( self.p1.addVars(self.gas_line_num,    T, K, name='weymouth_right_aux2',   lb=-1 * INF, ub=INF ) )
        self.pccp_relax                           = \
            tonp( self.p1.addVars(self.gas_line_num,    T, K, name='pccp_relax',            lb=-1 * INF, ub=INF ) )
        self.gas_linepack                         = \
            tonp( self.p1.addVars(self.gas_line_num,    T, K, name='gas_linepack',          lb=-1 * INF, ub=INF ) )
        self.p1_vars.extend(self.upper_gas_well_output.flatten().tolist())
        self.p1_vars.extend(self.lower_gas_well_output.flatten().tolist())
        self.p1_vars.extend(self.gas_node_pressure.flatten().tolist())
        self.p1_vars.extend(self.gas_flow_in.flatten().tolist())
        self.p1_vars.extend(self.gas_flow_out.flatten().tolist())
        self.p1_vars.extend(self.gas_linepack.flatten().tolist())
        self.p1_vars.extend(self.aux_weymouth_left.flatten().tolist())
        self.p1_vars.extend(self.aux_weymouth_right_1.flatten().tolist())
        self.p1_vars.extend(self.aux_weymouth_right_2.flatten().tolist())
        self.p1_vars.extend(self.pccp_relax          .flatten().tolist())


        # 对偶变量，全是空，之后添加到 p2 中去，之后 我 再代理 这些 变量
        self.dual_node_gas_balance                = np.empty((self.gas_node_num,           T, K, ), dtype=object)
        self.dual_linepack_with_pressure          = np.empty((self.gas_line_num,           T, K, ), dtype=object)
        self.dual_linepack_with_time              = np.empty((self.gas_line_num,           T, K, ), dtype=object)
        self.dual_compressor_consume              = np.empty((self.gas_line_num,           T, K, ), dtype=object)
        ...
        self.p2_vars.extend(self.dual_node_gas_balance.flatten().tolist())
        self.p2_vars.extend(self.dual_linepack_with_pressure.flatten().tolist())
        self.p2_vars.extend(self.dual_linepack_with_time.flatten().tolist())
        self.p2_vars.extend(self.dual_compressor_consume.flatten().tolist())
        self.p2_vars.extend(self.dual_compressor_pressure_up.flatten().tolist())
        self.p2_vars.extend(self.dual_well_upper_output_min.flatten().tolist())
        self.p2_vars.extend(self.dual_well_upper_output_max.flatten().tolist())
        self.p2_vars.extend(self.dual_well_lower_output_min.flatten().tolist())
        self.p2_vars.extend(self.dual_well_lower_output_max.flatten().tolist())
        self.p2_vars.extend(self.dual_gas_node_pressure_min.flatten().tolist())
        self.p2_vars.extend(self.dual_gas_node_pressure_max.flatten().tolist())
        self.p2_vars.extend(self.dual_gas_flow_in_and_out_great_zero.flatten().tolist())
        self.p2_vars.extend(self.dual_weymouth_aux_left.flatten().tolist())
        self.p2_vars.extend(self.dual_weymouth_relax_left_left.flatten().tolist())
        self.p2_vars.extend(self.dual_weymouth_relax_left_right.flatten().tolist())
        self.p2_vars.extend(self.dual_gas_flow_in_min.flatten().tolist())
        self.p2_vars.extend(self.dual_gas_flow_in_max.flatten().tolist())
        self.p2_vars.extend(self.dual_gas_flow_out_min.flatten().tolist())
        self.p2_vars.extend(self.dual_gas_flow_out_max.flatten().tolist())
        self.p2_vars.extend(self.dual_pccp_relax_great_zero.flatten().tolist())
        self.p2_vars.extend(self.dual_well_upper_capacity.flatten().tolist())
    ...
    def build_heat_system(self):
        ...
        self.upper_chp_heat_quoted_price_tuple_dict = \
            self.p2.addVars(self.chp_upper_num, T, lb=0, name='upper_chp_heat_quoted_price')
        ...
        self.upper_chp_gas_quoted_price_tuple_dict  = \
            self.p2.addVars(self.chp_upper_num, T, lb=0, name='upper_chp_gas_quoted_price')
        ...
        self.upper_chp_point                    = \
            tonp( self.p1.addVars(self.chp_upper_num, self.chp_point_num, T, K, name='upper_chp_point',  lb=-1 * INF, ub=INF ) )
        self.lower_chp_point                    = \
            tonp( self.p1.addVars(self.chp_lower_num, self.chp_point_num, T, K, name='lower_chp_point',  lb=-1 * INF, ub=INF ) )
        self.upper_chp_heat_output              = \
            tonp( self.p1.addVars(self.chp_upper_num, T, K, name='upper_chp_heat_output',                lb=-1 * INF, ub=INF ) )
        self.lower_chp_heat_output              = \
            tonp( self.p1.addVars(self.chp_lower_num, T, K, name='lower_chp_heat_output',                lb=-1 * INF, ub=INF ) )
        self.heat_node_tempe_supply             = \
            tonp( self.p1.addVars(self.heat_node_num, T, K, name='heat_node_tempe_supply',               lb=-1 * INF, ub=INF ) )
        self.heat_node_tempe_return             = \
            tonp( self.p1.addVars(self.heat_node_num, T, K, name='heat_node_tempe_return',               lb=-1 * INF, ub=INF ) )
        self.heat_pipe_start_tempe_supply       = \
            tonp( self.p1.addVars(self.heat_pipe_num, T, K, name='heat_pipe_start_tempe_supply_network', lb=-1 * INF, ub=INF ) )
        self.heat_pipe_end_tempe_supply         = \
            tonp( self.p1.addVars(self.heat_pipe_num, T, K, name='heat_pipe_end_tempe_supply_network',   lb=-1 * INF, ub=INF ) )
        self.heat_pipe_start_tempe_return       = \
            tonp( self.p1.addVars(self.heat_pipe_num, T, K, name='heat_pipe_start_tempe_return_network', lb=-1 * INF, ub=INF ) )
        self.heat_pipe_end_tempe_return         = \
            tonp( self.p1.addVars(self.heat_pipe_num, T, K, name='heat_pipe_end_tempe_return_network',   lb=-1 * INF, ub=INF ) )

        self.p1_vars.extend(self.upper_chp_point.flatten().tolist())
        self.p1_vars.extend(self.lower_chp_point.flatten().tolist())
        self.p1_vars.extend(self.upper_chp_heat_output.flatten().tolist())
        self.p1_vars.extend(self.lower_chp_heat_output.flatten().tolist())
        self.p1_vars.extend(self.heat_node_tempe_supply.flatten().tolist())
        self.p1_vars.extend(self.heat_node_tempe_return.flatten().tolist())
        self.p1_vars.extend(self.heat_pipe_start_tempe_supply.flatten().tolist())
        self.p1_vars.extend(self.heat_pipe_end_tempe_supply.flatten().tolist())
        self.p1_vars.extend(self.heat_pipe_start_tempe_return.flatten().tolist())
        self.p1_vars.extend(self.heat_pipe_end_tempe_return.flatten().tolist())
        ...
        self.p2_vars.extend(self.dual_lower_chp_point_sum_one   . flatten().tolist())
        self.p2_vars.extend(self.dual_lower_chp_heat_output     . flatten().tolist())
        self.p2_vars.extend(self.dual_upper_chp_heat_output     . flatten().tolist())
        self.p2_vars.extend(self.dual_upper_chp_point_sum_one   . flatten().tolist())
        self.p2_vars.extend(self.dual_lower_chp_point_less_one  . flatten().tolist())
        self.p2_vars.extend(self.dual_lower_chp_point_great_zero. flatten().tolist())
        self.p2_vars.extend(self.dual_upper_chp_point_less_one  . flatten().tolist())
        self.p2_vars.extend(self.dual_upper_chp_point_great_zero. flatten().tolist())
        self.p2_vars.extend(self.dual_exchanger_balance         . flatten().tolist())
        self.p2_vars.extend(self.dual_heater_supply_min         . flatten().tolist())
        self.p2_vars.extend(self.dual_heater_supply_max         . flatten().tolist())
        self.p2_vars.extend(self.dual_heater_return_min         . flatten().tolist())
        self.p2_vars.extend(self.dual_heater_return_max         . flatten().tolist())
        self.p2_vars.extend(self.dual_exchanger_supply_min      . flatten().tolist())
        self.p2_vars.extend(self.dual_exchanger_supply_max      . flatten().tolist())
        self.p2_vars.extend(self.dual_exchanger_return_min      . flatten().tolist())
        self.p2_vars.extend(self.dual_exchanger_return_max      . flatten().tolist())
        self.p2_vars.extend(self.dual_heater_balance            . flatten().tolist())
        ...
        for k in range(K):
            for chp in range(self.chp_upper_num):
                for t in range(T):
                    ...
                    self.dual_upper_chp_heat_output[chp, t, k], c1 = Complementary_equal(
                        1 * cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'upper_chp_heat_output[' + str(chp) + ',' + str(t) + ',' + str(k) + ']')
                    self.dual_upper_chp_point_sum_one[chp, t, k], c2 = Complementary_equal(
                        cons_expr3, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            for chp in range(self.chp_lower_num):
                for t in range(T):
                    ...
                    self.dual_lower_chp_heat_output[chp, t, k], c1 = Complementary_equal(
                        1 * cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'lower_chp_heat_output[' + str(chp) + ',' + str(t) + ',' + str(k) + ']')
                    self.dual_lower_chp_point_sum_one[chp, t, k], c2 = Complementary_equal(
                        cons_expr3, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            for chp in range(self.chp_upper_num):
                for p in range(self.chp_point_num):
                    for t in range(T):
                        ...
                        self.dual_upper_chp_point_great_zero[chp, p, t, k], c1 = Complementary_great(
                            cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                            'upper_chp_point_great_zero[' + str(chp) + ',' + str(p) + ',' + str(t) + ',' + str(k) + ']')
                        self.dual_upper_chp_point_less_one[chp, p, t, k], c2 = Complementary_great(
                            cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            for chp in range(self.chp_lower_num):
                for p in range(self.chp_point_num):
                    for t in range(T):
                        ...
                        self.dual_lower_chp_point_great_zero[chp, p, t, k], c1 = Complementary_great(
                            cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                            'lower_chp_point_great_zero[' + str(chp) + ',' + str(p) + ',' + str(t) + ',' + str(k) + ']')
                        self.dual_lower_chp_point_less_one[chp, p, t, k], c2 = Complementary_great(
                            cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            for heater in range(self.heat_heater_num):
                for t in range(T):
                    ...
                    self.dual_heater_balance[heater, t, k], c1 = Complementary_equal(
                        1 * cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            for exchanger in range(self.heat_exchanger_num):
                for t in range(T):
                    ...
                    self.dual_exchanger_balance[exchanger, t, k], c1 = Complementary_equal(
                        1*cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            for line in range(self.heat_pipe_num):
                for t in range(T):
                    ...
                    self.dual_line_temp_start_supply[line, t, k], c1 = Complementary_equal(
                        1 * cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'line_temp_start_supply[' + str(line) + str(t) + ',' + str(k) + ']')
                    self.dual_lien_temp_start_return[line, t, k], c2 = Complementary_equal(
                        1 * cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            for node in range(self.heat_node_num):      # mix constrains
                for t in range(T):
                    ...
                    self.mix_constraints_supply[line, t, k], c1 = Complementary_equal(cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                                            'mix_constraints_supply[' + str(node) + str(t) + ',' + str(k) + ']')
                    self.mix_constraints_return[line, t, k], c2 = Complementary_equal(cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            for line in range(self.heat_pipe_num):
                for t in range(T):
                    ...
                    self.dual_heat_loss_supply[line, t, k], c1 = Complementary_equal(cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                                            'heat_loss_supply[' + str(line) + str(t) + ',' + str(k) + ']')
                    self.dual_heat_loss_return[line, t, k], c2 = Complementary_equal(cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            for heater in range(self.heat_heater_num):
                for t in range(T):
                    ...
                    self.dual_heater_supply_min[heater, t, k], c1 = Complementary_great(
                        cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'heater_supply_min[' + str(heater) + str(t) + ',' + str(k) + ']')
                    self.dual_heater_supply_max[heater, t, k], c2 = Complementary_great(
                        cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'heater_supply_max[' + str(heater) + str(t) + ',' + str(k) + ']')
                    self.dual_heater_return_min[heater, t, k], c3 = Complementary_great(
                        cons_expr3, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'heater_return_min[' + str(heater) + str(t) + ',' + str(k) + ']')
                    self.dual_heater_return_max[heater, t, k], c4 = Complementary_great(
                        cons_expr4, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            for exchanger in range(self.heat_exchanger_num):
                for t in range(T):
                    ...
                    self.dual_exchanger_supply_min[exchanger, t, k], c1 = Complementary_great(
                        cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'exchanger_supply_min[' + str(exchanger) + str(t) + ',' + str(k) + ']')
                    self.dual_exchanger_supply_max[exchanger, t, k], c2 = Complementary_great(
                        cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'exchanger_supply_max[' + str(exchanger) + str(t) + ',' + str(k) + ']')
                    self.dual_exchanger_return_min[exchanger, t, k], c3 = Complementary_great(
                        cons_expr3, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'exchanger_return_min[' + str(exchanger) + str(t) + ',' + str(k) + ']')
                    self.dual_exchanger_return_max[exchanger, t, k], c4 = Complementary_great(
                        cons_expr4, self.p1, self.p2, self.DE[k], self.Dobj[k],
        ...
        for k in range(K):
            for node in range(self.gas_node_num):
                for t in range(T):
                    ...
                    self.dual_node_gas_balance[node, t, k], C = Complementary_equal(
                        1*cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'node_gas_balance[' + str(node) + ',' + str(t) + ',' + str(k) + ']')
                    self.C.append(C)
                    # self.p1, self.p2.addConstr(self.dual_node_gas_balance[node, t, k] >= 0)
        ...
        for k in range(K):
            for well in range(self.well_upper_num):
                for t in range(T):
                    ...
                    self.dual_well_upper_capacity[well, t, k], C = Complementary_great(
                        cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            for well in range(self.well_lower_num):
                for t in range(T):
                    ...
                    self.share[well, t, k], C = Complementary_great(
                        cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
        ...
        for k in range(K):
            for line in self.gas_inactive_line:
                for t in range(0, T-1):
                    ...
                    self.dual_linepack_with_pressure[line, t, k], C1 = Complementary_equal(
                        cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'gas_linepack_equation[' + str(line) + ',' + str(t) + ',' + str(k) + ']')
                    self.dual_linepack_with_time[line, t, k], C2 = Complementary_equal(
                        cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            #             Complementary_equal(cons_expr1, self.p1, self.p2,
            ...
            #             Complementary_great(cons_expr1, self.p1, self.p2,
        ...
        for k in range(K):
            for well in range(self.well_upper_num):
                for t in range(T):
                    ...
                    self.dual_well_upper_output_min[well, t, k], C1 = Complementary_great(
                        cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'upper_well_output_min[' + str(well) + ',' + str(t) + ',' + str(k) + ']')
                    self.dual_well_upper_output_max[well, t, k], C2 = Complementary_great(
                        cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            for well in range(self.well_lower_num):
                for t in range(T):
                    ...
                    self.dual_well_lower_output_min[well, t, k], C1 = Complementary_great(
                        cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'lower_well_output_min[' + str(well) + ',' + str(t) + ',' + str(k) + ']')
                    self.dual_well_lower_output_max[well, t, k], C2 = Complementary_great(
                        cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
        ...
        for k in range(K):
            for node in range(self.gas_node_num):
                for t in range(T):
                    ...
                    self.dual_gas_node_pressure_min[node, t, k], C1 = Complementary_great(
                        cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'node_pressure_min[' + str(node) + ',' + str(t) + ',' + str(k) + ']')
                    self.dual_gas_node_pressure_max[node, t, k], C2 = Complementary_great(
                        cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
        ...
        for k in range(K):
            for line in range(self.gas_line_num):
                for t in range(T):
                    ...
                    self.dual_gas_flow_in_min[line, t, k], C1  = Complementary_great(
                        cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'gas_flow_in_min['  + str(line) + ',' + str(t) + ',' + str(k) + ']')
                    self.dual_gas_flow_in_max[line, t, k], C2  = Complementary_great(
                        cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'gas_flow_in_max['  + str(line) + ',' + str(t) + ',' + str(k) + ']')
                    self.dual_gas_flow_out_min[line, t, k], C3 = Complementary_great(
                        cons_expr3, self.p1, self.p2, self.DE[k], self.Dobj[k],
                        'gas_flow_out_min[' + str(line) + ',' + str(t) + ',' + str(k) + ']')
                    self.dual_gas_flow_out_max[line, t, k], C4 = Complementary_great(
                        cons_expr4, self.p1, self.p2, self.DE[k], self.Dobj[k],
        ...
        for k in range(K):
            for line in range(self.gas_line_num):
                for t in range(T):
                    ...
                    self.dual_pccp_relax_great_zero[line, t, k], C1 = Complementary_great(
                        cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
        ...
        for k in range(K):
            for line in self.gas_inactive_line:
                for t in range(T):
                    ...
                    self.dual_weymouth_aux_left[line, t, k], C1 = Complementary_equal(
                        cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
            ...
            for line in self.gas_inactive_line:
                for t in range(T):
                    ...
                    measurement = Complementary_soc(
                        ...
                        [self.gas_node_pressure[self.gas_pipe_start_node[line], t, k]],
                        self.p1, self.p2, self.DE[k], self.Dobj[k],
        ...
        for k in range(K):
            for line in self.gas_inactive_line:
                for t in range(T):
                    ...
                    self.dual_weymouth_aux_right1[line, t, k], C1 = \
                        Complementary_equal(cons_expr1, self.p1, self.p2, self.DE[k], self.Dobj[k],
                                                 'weymouth_right_aux1[' + str(line) + ',' + str(t) + ',' + str(k) + ']')
                    self.dual_weymouth_aux_right2[line, t, k], C2 = \
                        Complementary_equal(cons_expr2, self.p1, self.p2, self.DE[k], self.Dobj[k],
        ...
        for k in range(K):
            for line in self.gas_inactive_line:
                for t in range(T):
                    ...
                    self.dual_weymouth_relax_right_left[line, t, k], self.dual_weymouth_relax_right_right[line, t, k], measure = \
                        Complementary_soc(
                        [2 * d, 1],
                        [self.gas_node_pressure[self.gas_pipe_start_node[line], t, k],
    ...
    def build_kkt_derivative_constraints(self, penalty):
        ...
        # 这个 kkt 我 加到 p2 里面去
        self.model.update()
        for var in self.p1_vars:
            expr = my_expr.getCoeff(var)
            my_expr.addConstr(expr, self.p2, var.VarName)
