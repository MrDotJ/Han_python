...
class OneLayer:
    ...
    def build_power_system(self):
        ...
        self.upper_generator_power_output = \
            tonp(self.model.addVars(self.generator_upper_num, T, K, name='upper_generator_power', lb=-1 * INFINITY,
                                    ub=INFINITY))
        self.lower_generator_power_output = \
            tonp(self.model.addVars(self.generator_lower_num, T, K, name='lower_generator_power', lb=-1 * INFINITY,
                                    ub=INFINITY))
    ...
    def build_gas_system(self):
        ...
        self.upper_gas_well_output = tonp(
            self.model.addVars(self.well_upper_num, T, K, name='upper_well_output', lb=-1 * INFINITY, ub=INFINITY))
        self.lower_gas_well_output = tonp(
            self.model.addVars(self.well_lower_num, T, K, name='lower_well_output', lb=-1 * INFINITY, ub=INFINITY))
        self.gas_node_pressure = tonp(
            self.model.addVars(self.gas_node_num, T, K, name='gas_node_pressure', lb=-1 * INFINITY, ub=INFINITY))
        self.gas_flow_in = tonp(
            self.model.addVars(self.gas_line_num, T, K, name='gas_flow_in', lb=-1 * INFINITY, ub=INFINITY))
        self.gas_flow_out = tonp(
            self.model.addVars(self.gas_line_num, T, K, name='gas_flow_out', lb=-1 * INFINITY, ub=INFINITY))
        ...
        self.aux_weymouth_right_2 = tonp(
            self.model.addVars(self.gas_line_num, T, K, name='weymouth_right_auxiliary2', lb=-1 * INFINITY,
                               ub=INFINITY))
        self.pccp_relax = tonp(
            self.model.addVars(self.gas_line_num, T, K, name='pccp_relax', lb=-1 * INFINITY, ub=INFINITY))
        self.gas_linepack = tonp(
            self.model.addVars(self.gas_line_num, T, K, name='gas_linepack', lb=-1 * INFINITY, ub=INFINITY))
    ...
    def build_heat_system(self):
        ...
        self.upper_chp_gas_quoted_price_tuple_dict = self.model.addVars(self.chp_upper_num, T, lb=0,
                                                                        name='upper_chp_gas_quoted_price')
        ...
        self.upper_chp_point = tonp(
            self.model.addVars(self.chp_upper_num, self.chp_point_num, T, K, name='upper_chp_point', lb=-1 * INFINITY,
                               ub=INFINITY))
        self.lower_chp_point = tonp(
            self.model.addVars(self.chp_lower_num, self.chp_point_num, T, K, name='lower_chp_point', lb=-1 * INFINITY,
                               ub=INFINITY))
        self.upper_chp_heat_output = tonp(
            self.model.addVars(self.chp_upper_num, T, K, name='upper_chp_heat_output', lb=-1 * INFINITY, ub=INFINITY))
        self.lower_chp_heat_output = tonp(
            self.model.addVars(self.chp_lower_num, T, K, name='lower_chp_heat_output', lb=-1 * INFINITY, ub=INFINITY))
        self.heat_node_tempe_supply = tonp(
            self.model.addVars(self.heat_node_num, T, K, name='heat_node_temperature_supply', lb=-1 * INFINITY,
                               ub=INFINITY))
        self.heat_node_tempe_return = tonp(
            self.model.addVars(self.heat_node_num, T, K, name='heat_node_temperature_return', lb=-1 * INFINITY,
                               ub=INFINITY))
        ...
        for node in range(self.ele_node_num):
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_node_power_balance[node, t, k] = \
                        Complementary_equal(1 * cons_expr1, self.model, name1, dual_expr)
        # 直流潮流
        for line in range(self.ele_line_num):
            for t in range(T):
                for k in range(K):
                    ...
                    _ = Complementary_equal(cons_expr1, self.model, name1, dual_expr)
        # 参考电节点
        for t in range(T):
            for k in range(K):
        ...
        for node in range(self.ele_node_num):
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_bus_angle_min[node, t, k] = Complementary_great(cons_expr1, self.model, name1, dual_expr)
                    self.dual_bus_angle_max[node, t, k] = Complementary_great(cons_expr2, self.model, name2, dual_expr)
        # 线路功率范围
        for line in range(self.ele_line_num):
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_line_power_flow_great[line, t, k] = \
                        Complementary_great(cons_expr1, self.model, name1, dual_expr)
                    self.dual_line_power_flow_less[line, t, k] = \
                        Complementary_great(cons_expr2, self.model, name2, dual_expr)
        # 上层 发电机 辐值 上下限
        for gen in range(self.generator_upper_num):
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_upper_generator_power_output_min[gen, t, k] = \
                        Complementary_great(cons_expr1, self.model, name1, dual_expr)
                    self.dual_upper_generator_power_output_max[gen, t, k] = \
                        Complementary_great(cons_expr2, self.model, name2, dual_expr)
        # 下层 发电机 辐值 上下限
        for gen in range(self.generator_lower_num):
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_lower_generator_power_output_min[gen, t, k] = \
                        Complementary_great(cons_expr1, self.model, name1, dual_expr)
                    self.dual_lower_generator_power_output_max[gen, t, k] = \
                        Complementary_great(cons_expr2, self.model, name2, dual_expr)
        # 上层 发电机 爬坡约束 0 - T-2
        for gen in range(self.generator_upper_num):
            for t in range(T - 1):
                for k in range(K):
                    ...
                    _ = Complementary_great(cons_expr1, self.model, name1, dual_expr)
                    _ = Complementary_great(cons_expr2, self.model, name2, dual_expr)
        # 下层 发电机 爬坡约束 0 - T-2
        for gen in range(self.generator_lower_num):
            for t in range(T - 1):
                for k in range(K):
                    ...
                    name2 = 'dual_lower_generator_power_output_ramp_down' + str(t) + '_' + str(gen) + 'scenario' + str(k)
                    self.dual_lower_generator_power_output_ramp_up[gen, t, k] = \
                        Complementary_great(cons_expr1, self.model, name1, dual_expr)
                    self.dual_lower_generator_power_output_ramp_down[gen, t, k] = \
                        Complementary_great(cons_expr2, self.model, name2, dual_expr)
        # 上层 发电机 爬坡约束 T-1
        for gen in range(self.generator_upper_num):
            for t in [T - 1]:
            ...
            for t in range(T):
                for k in range(K):
                    cons_expr2 = ((self.upper_chp_point[chp, :, t, k].reshape((1, -1))).dot(
                        self.upper_chp_HEAT[chp, :].reshape((-1, 1))))[0][0] - \
                    ...
                    self.dual_upper_chp_heat_output[chp, t, k] = Complementary_equal(
                        -1 * cons_expr2, self.model,
                        'dual_upper_chp_heat_output_' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)
                    self.dual_upper_chp_point_sum_one[chp, t, k] = Complementary_equal(
                        cons_expr3, self.model, 'dual_upper_chp_point_sum_one' + str(t) + '_' + str(chp) + 'S' + str(k),
                        dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    cons_expr2 = ((self.lower_chp_point[chp, :, t, k].reshape((1, -1))).dot(
                        self.lower_chp_HEAT[chp, :].reshape((-1, 1))))[0][0] - \
                    ...
                    self.dual_lower_chp_heat_output[chp, t, k] = Complementary_equal(
                        -1 * cons_expr2, self.model,
                        'dual_lower_chp_heat_output_' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)
                    self.dual_lower_chp_point_sum_one[chp, t, k] = Complementary_equal(
                        cons_expr3, self.model, 'dual_lower_chp_point_sum_one' + str(t) + '_' + str(chp) + 'S' + str(k),
                        dual_expr)
                ...
                for t in range(T):
                    for k in range(K):
                        ...
                        self.dual_upper_chp_point_great_zero[chp, point, t, k] = Complementary_great(
                            cons_expr1, self.model,
                            'dual_upper_chp_point_great_zero' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)
                        self.dual_upper_chp_point_less_one[chp, point, t, k] = Complementary_great(
                            cons_expr2, self.model,
                            'dual_upper_chp_point_less_one' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)
                ...
                for t in range(T):
                    for k in range(K):
                        ...
                        self.dual_lower_chp_point_great_zero[chp, point, t, k] = Complementary_great(
                            cons_expr1, self.model,
                            'dual_lower_chp_point_great_zero' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)
                        self.dual_lower_chp_point_less_one[chp, point, t, k] = Complementary_great(
                            cons_expr2, self.model,
                            'dual_lower_chp_point_less_one' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_heater_balance[heater, t, k] = Complementary_equal(
                        1 * cons_expr1, self.model, 'dual_heater_balance' + str(t) + '_' + str(heater) + 'S' + str(k),
                        dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_exchanger_balance[exchanger, t, k] = Complementary_equal(
                        -1 * cons_expr1, self.model, 'dual_exchanger_balance' + str(t) + '_' + str(exchanger) + str(k),
                        dual_expr)
        ...
        for line in range(self.heat_pipe_num):
            for t in range(T):
                for k in range(K):
                    ...
                    _ = Complementary_equal(-1 * cons_expr1, self.model,
                                            'dual_line_temp_start_supply_' + str(t) + '_' + str(line) + 'S' + str(k),
                                            dual_expr)
                    _ = Complementary_equal(-1 * cons_expr2, self.model,
        ...
        for node in range(self.heat_node_num):  # mix constrains
            for t in range(T):
                for k in range(K):
                    ...
                    _ = Complementary_equal(cons_expr1, self.model,
                                            'dual_mix_constraints_supply' + str(t) + '_' + str(node) + 'S' + str(k),
                                            dual_expr)
                    _ = Complementary_equal(cons_expr2, self.model,
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_heater_supply_min[heater, t, k] = Complementary_great(
                        cons_expr1, self.model, 'dual_heater_supply_min' + str(t) + '_' + str(heater) + 'S' + str(k),
                        dual_expr)
                    self.dual_heater_supply_max[heater, t, k] = Complementary_great(
                        cons_expr2, self.model, 'dual_heater_supply_max' + str(t) + '_' + str(heater) + 'S' + str(k),
                        dual_expr)
                    self.dual_heater_return_min[heater, t, k] = Complementary_great(
                        cons_expr3, self.model, 'dual_heater_return_min' + str(t) + '_' + str(heater) + 'S' + str(k),
                        dual_expr)
                    self.dual_heater_return_max[heater, t, k] = Complementary_great(
                        cons_expr4, self.model, 'dual_heater_return_max' + str(t) + '_' + str(heater) + 'S' + str(k),
                        dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_exchanger_supply_min[exchanger, t, k] = Complementary_great(
                        cons_expr1, self.model,
                        'dual_exchanger_supply_min' + str(t) + '_' + str(exchanger) + 'S' + str(k), dual_expr)
                    self.dual_exchanger_supply_max[exchanger, t, k] = Complementary_great(
                        cons_expr2, self.model,
                        'dual_exchanger_supply_max' + str(t) + '_' + str(exchanger) + 'S' + str(k), dual_expr)
                    self.dual_exchanger_return_min[exchanger, t, k] = Complementary_great(
                        cons_expr3, self.model,
                        'dual_exchanger_return_min' + str(t) + '_' + str(exchanger) + 'S' + str(k), dual_expr)
                    self.dual_exchanger_return_max[exchanger, t, k] = Complementary_great(
                        cons_expr4, self.model,
                        'dual_exchanger_return_max' + str(t) + '_' + str(exchanger) + 'S' + str(k), dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_node_gas_balance[node, t, k] = Complementary_equal(
                        1 * cons_expr1, self.model,
                        'dual_node_gas_balance_time_' + str(t) + '_node_' + str(node) + 'scenario_' + str(k), dual_expr)
            ...
            for t in range(0, T - 1):
                for k in range(K):
                    cons_expr1 = self.gas_linepack[line, t, k] - self.gas_linepack_coeff[line] * (
                            self.gas_node_pressure[self.gas_pipe_start_node[line], t, k] + self.gas_node_pressure[
                        self.gas_pipe_end_node[line], t, k]) / 2
                    cons_expr2 = self.gas_linepack[line, t + 1, k] - self.gas_linepack[line, t, k] - self.gas_flow_in[
                        line, t, k] + self.gas_flow_out[line, t, k]
                    self.dual_linepack_with_pressure[line, t, k] = Complementary_equal(
                        cons_expr1, self.model,
                        'dual_gas_linepack_equation_line_' + str(line) + '_t_' + str(t) + '_scenario_' + str(k),
                        dual_expr)
                    self.dual_linepack_with_time[line, t, k] = Complementary_equal(
                        cons_expr2, self.model,
                        'dual_gas_linepack_with_time_line_' + str(line) + '_t_' + str(t) + '_scenario_' + str(k),
                        dual_expr)
            ...
            for t in [T - 1]:
                for k in range(K):
                    cons_expr1 = self.gas_linepack[line, t, k] - self.gas_linepack_coeff[line] * (
                            self.gas_node_pressure[self.gas_pipe_start_node[line], t, k] + self.gas_node_pressure[
                        self.gas_pipe_end_node[line], t, k]) / 2
                    cons_expr2 = self.gas_linepack[line, 0, k] - self.gas_linepack[line, t, k] - self.gas_flow_in[
                        line, t, k] + self.gas_flow_out[line, t, k]
                    self.dual_linepack_with_pressure[line, t, k] = Complementary_equal(
                        cons_expr1, self.model,
                        'dual_gas_linepack_equation_line_' + str(line) + '_t_' + str(t) + '_scenario_' + str(k),
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_well_upper_output_min[well, t, k] = Complementary_great(
                        cons_expr1, self.model,
                        'dual_upper_well_output_min_' + str(well) + '_t_' + str(t) + '_scenario_' + str(k), dual_expr)
                    self.dual_well_upper_output_max[well, t, k] = Complementary_great(
                        cons_expr2, self.model,
                        'dual_upper_well_output_max_' + str(well) + '_t_' + str(t) + '_scenario_' + str(k), dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_well_lower_output_min[well, t, k] = Complementary_great(
                        cons_expr1, self.model,
                        'dual_lower_well_output_min_' + str(well) + '_t_' + str(t) + '_scenario_' + str(k), dual_expr)
                    self.dual_well_lower_output_max[well, t, k] = Complementary_great(
                        cons_expr2, self.model,
                        'dual_lower_well_output_max_' + str(well) + '_t_' + str(t) + '_scenario_' + str(k), dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_gas_node_pressure_min[node, t, k] = Complementary_great(
                        cons_expr1, self.model,
                        'dual_node_pressure_min_' + str(node) + '_t_' + str(t) + '_scenario_' + str(k), dual_expr)
                    self.dual_gas_node_pressure_max[node, t, k] = Complementary_great(
                        cons_expr2, self.model,
                        'dual_node_pressure_max_' + str(node) + '_t_' + str(t) + '_scenario_' + str(k), dual_expr)
        #
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_gas_flow_in_min[line, t, k] = Complementary_great(
                        cons_expr1, self.model,
                        'dual_gas_flow_in_min' + str(line) + '_t_' + str(t) + '_scenario_' + str(k), dual_expr)
                    self.dual_gas_flow_in_max[line, t, k] = Complementary_great(
                        cons_expr2, self.model,
                        'dual_gas_flow_in_max' + str(line) + '_t_' + str(t) + '_scenario_' + str(k), dual_expr)
                    self.dual_gas_flow_out_min[line, t, k] = Complementary_great(
                        cons_expr3, self.model,
                        'dual_gas_flow_out_min' + str(line) + '_t_' + str(t) + '_scenario_' + str(k), dual_expr)
                    self.dual_gas_flow_out_max[line, t, k] = Complementary_great(
                        cons_expr4, self.model,
                        'dual_gas_flow_out_max' + str(line) + '_t_' + str(t) + '_scenario_' + str(k), dual_expr)
        #
        for line in range(self.gas_line_num):
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_pccp_relax_great_zero[line, t, k] = Complementary_great(
                        cons_expr1, self.model,
                        'dual_pccp_relax_great_zero' + str(line) + '_t_' + str(t) + '_scenario_' + str(k), dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_weymouth_aux_left[line, t, k] = Complementary_equal(
                        ...
                        dual_expr)
                    self.dual_weymouth_relax_left_left[line, t, k], self.dual_weymouth_relax_left_right[
                        line, t, k] = Complementary_soc(
                        ...
                        self.model,
                        'weymouth_relax_left_soc_' + str(line) + '_t_' + str(t) + '_scenario_' + str(k), dual_expr)

        # for line in self.gas_inactive_line:
        #     for t in range(T):
        #         for k in range(K):
        #             cons_expr1 = self.gas_weymouth[line] * (self.gas_node_pressure[self.gas_pipe_start_node[line], t, k] -
        #                                                     self.gas_node_pressure[self.gas_pipe_end_node[line], t, k]) - \
        #                          (self.gas_flow_in[line, t, k] + self.gas_flow_out[line, t, k])
        #             _ = Complementary_equal(cons_expr1, self.model, 'dual_weymouth_simplify' + str(line) + str(t) + str(k), dual_expr)
        ...
        for k in range(K):
            ...
            for line in self.gas_inactive_line:
                for t in range(T):
                    ...
                    dual_vars1, constr1, expr1 = Complementary_equal_plus(cons_expr1, self.model,
                                                                          'weymouth_relax_right_auxiliary1_' + str(
                                                                              line) + '_t_' + str(
                                                                              t) + '_scenario_' + str(k))
                    dual_vars2, constr2, expr2 = Complementary_equal_plus(cons_expr2, self.model,
                                                                          'weymouth_relax_right_auxiliary2_' + str(
                                                                              line) + '_t_' + str(
                                                                              t) + '_scenario_' + str(k))
                    # 构建SOC 约束
    ...
    def build_lower_objective(self):
        ...
        for gen in range(self.generator_upper_num):
            for time in range(T):
                for k in range(K):
                    lower_objs.append(self.upper_generator_quoted_price[gen, time] *
                                      self.upper_generator_power_output[gen, time, k])
        # 下层 发电机的 发电 成本
        for gen in range(self.generator_lower_num):
            for time in range(T):
                for k in range(K):
                    lower_objs.append(self.generator_lower_cost[gen] *
                                      self.lower_generator_power_output[gen, time, k])
        # CHP 从上层买热 quoted price 买
        for chp in range(self.chp_upper_num):
            for time in range(T):
                for k in range(K):
                    # lower_objs.append(self.upper_chp_power_output[chp, time, k] * self.upper_chp_power_quoted_price[chp, time])
                    lower_objs.append(self.upper_chp_power_output[chp, time, k] *
                                      self.upper_chp_power_quoted_price[chp, time])
                    lower_objs.append(self.upper_chp_heat_output[chp, time, k] *
                                      self.upper_chp_heat_quoted_price[chp, time])
        ...
        #     for time in range(T):
        #         for k in range(K):
        #             lower_objs.append(-1 * (self.upper_chp_heat_output[chp, time, k] * self.chp_upper_coeff_h_1[
        #                 chp]) * 1)  # self.upper_chp_gas_quoted_price[chp, time])
        ...
        for k in range(K):
            ...
            for chp in range(self.chp_upper_num):
                for t in range(T):
                    ...
                    expected_cost.append(-1 * (self.upper_chp_heat_output[chp, t, k] +
                                               self.upper_chp_power_output[chp, t, k]) * self.chp_upper_coeff_h_1[chp])  # 写法1
        ...
        for k in range(K):
            ...
            for chp in range(self.chp_upper_num):
                for t in range(T):
                    ...
                    objs_cost.append(self.chp_upper_coeff_p_2[chp] * self.upper_chp_power_output[chp, t, k] *
                                     self.upper_chp_power_output[chp, t, k])
                    objs_cost.append(self.chp_upper_coeff_h_1[chp] * self.upper_chp_heat_output[chp, t, k])
                    objs_cost.append(self.chp_upper_coeff_h_2[chp] * self.upper_chp_heat_output[chp, t, k] *
                                     self.upper_chp_heat_output[chp, t, k])
                    objs_cost.append(self.chp_upper_coeff_cross[chp] * self.upper_chp_heat_output[chp, t, k] *
                                     self.upper_chp_power_output[chp, t, k])
            for well in range(self.well_upper_num):
                for t in range(T):
            ...
            for chp in range(self.chp_lower_num):
                for t in range(T):
                    ...
                    objs_revenue.append(self.chp_lower_coeff_p_2[chp] * self.lower_chp_power_output[chp, t, k] *
                                        self.lower_chp_power_output[chp, t, k])
                    objs_revenue.append(self.chp_lower_coeff_h_1[chp] * self.lower_chp_heat_output[chp, t, k])
                    objs_revenue.append(self.chp_lower_coeff_h_2[chp] * self.lower_chp_heat_output[chp, t, k] *
                                        self.lower_chp_heat_output[chp, t, k])
                    objs_revenue.append(self.chp_lower_coeff_cross[chp] * self.lower_chp_heat_output[chp, t, k] *
                                        self.lower_chp_power_output[chp, t, k])
            for well in range(self.well_lower_num):
                for t in range(T):
            ...
            for node in range(self.gas_node_num):
                for t in range(T):
                    objs_revenue.append(
                        -1 * self.dual_gas_node_pressure_min[node, t, k] * self.gas_node_pressure_min[node])
                    objs_revenue.append(
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    weymouth_right.append(((self.gas_flow_in[line, t, k] + self.gas_flow_out[line, t, k]) *
                                           (self.gas_flow_in[line, t, k] + self.gas_flow_out[
                                               line, t, k])).getValue() / 4)
