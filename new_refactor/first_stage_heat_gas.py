...
class OneLayer:
    ...
    def build_heat_system_original_and_dual_constraints(self):
        ...
        for chp in range(self.chp_upper_num):
            for t in range(T):
                for k in range(K):
                    cons_expr2 = ( (self.upper_chp_point[chp, :, t, k].reshape((1, -1))).dot(self.upper_chp_HEAT[chp, :].reshape((-1, 1)))  )[0][0] - \
                    ...
                    self.dual_upper_chp_heat_output[chp, t, k] = Complementary_equal(
                        -1 * cons_expr2, self.model, 'dual_upper_chp_heat_output_' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)
                    self.dual_upper_chp_point_sum_one[chp, t, k] = Complementary_equal(
                        cons_expr3, self.model, 'dual_upper_chp_point_sum_one' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)

        for chp in range(self.chp_lower_num):
            for t in range(T):
                for k in range(K):
                    cons_expr2 = (  (self.lower_chp_point[chp, :, t, k].reshape((1, -1))).dot(self.lower_chp_HEAT[chp, :]. reshape((-1, 1)))   )[0][0] - \
                    ...
                    self.dual_lower_chp_heat_output[chp, t, k] = Complementary_equal(
                        -1 * cons_expr2, self.model, 'dual_lower_chp_heat_output_' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)
                    self.dual_lower_chp_point_sum_one[chp, t, k] = Complementary_equal(
                        cons_expr3, self.model, 'dual_lower_chp_point_sum_one' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)
                ...
                for t in range(T):
                    for k in range(K):
                        ...
                        self.dual_upper_chp_point_great_zero[chp, point, t, k] = Complementary_great(
                            cons_expr1, self.model, 'dual_upper_chp_point_great_zero' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)
                        self.dual_upper_chp_point_less_one[chp, point, t, k] = Complementary_great(
                            cons_expr2, self.model, 'dual_upper_chp_point_less_one' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)
                ...
                for t in range(T):
                    for k in range(K):
                        ...
                        self.dual_lower_chp_point_great_zero[chp, point, t, k] = Complementary_great(
                            cons_expr1, self.model, 'dual_lower_chp_point_great_zero' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)
                        self.dual_lower_chp_point_less_one[chp, point, t, k] = Complementary_great(
                            cons_expr2, self.model, 'dual_lower_chp_point_less_one' + str(t) + '_' + str(chp) + 'S' + str(k), dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_heater_balance[heater, t, k] = Complementary_equal(
                        1 * cons_expr1, self.model, 'dual_heater_balance' + str(t) + '_' + str(heater) + 'S' + str(k), dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_exchanger_balance[exchanger, t, k] = Complementary_equal(
                        -1*cons_expr1, self.model, 'dual_exchanger_balance' + str(t) + '_' + str(exchanger) + str(k), dual_expr)
        ...
        for line in range(self.heat_pipe_num):
            for t in range(T):
                for k in range(K):
                    ...
                    _ = Complementary_equal(-1 * cons_expr1, self.model, 'dual_line_temp_start_supply_' + str(t) + '_' + str(line) + 'S' + str(k), dual_expr)
                    _ = Complementary_equal(-1 * cons_expr2, self.model, 'dual_line_temp_start_return' + str(t) + '_' + str(line) + 'S' + str(k), dual_expr)
        ...
        for node in range(self.heat_node_num):      # mix constrains
            for t in range(T):
                for k in range(K):
                    ...
                    _ = Complementary_equal(cons_expr1, self.model, 'dual_mix_constraints_supply' + str(t) + '_' + str(node) + 'S' + str(k), dual_expr)
                    _ = Complementary_equal(cons_expr2, self.model, 'dual_mix_constraints_return' + str(t) + '_' + str(node) + 'S' + str(k), dual_expr)
        ...
        for line in range(self.heat_pipe_num):
            for t in range(T):
                for k in range(K):
                    ...
                    _ = Complementary_equal(cons_expr1, self.model, 'dual_heat_loss_supply' + str(t) + '_' + str(line) + 'S' + str(k), dual_expr)
                    _ = Complementary_equal(cons_expr2, self.model, 'dual_heat_loss_return' + str(t) + '_' + str(line) + 'S' + str(k), dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_heater_supply_min[heater, t, k] = Complementary_great(
                        cons_expr1, self.model, 'dual_heater_supply_min' + str(t) + '_' + str(heater) + 'S' + str(k), dual_expr)
                    self.dual_heater_supply_max[heater, t, k] = Complementary_great(
                        cons_expr2, self.model, 'dual_heater_supply_max' + str(t) + '_' + str(heater) + 'S' + str(k), dual_expr)
                    self.dual_heater_return_min[heater, t, k] = Complementary_great(
                        cons_expr3, self.model, 'dual_heater_return_min' + str(t) + '_' + str(heater) + 'S' + str(k), dual_expr)
                    self.dual_heater_return_max[heater, t, k] = Complementary_great(
                        cons_expr4, self.model, 'dual_heater_return_max' + str(t) + '_' + str(heater) + 'S' + str(k), dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_exchanger_supply_min[exchanger, t, k] = Complementary_great(
                        cons_expr1, self.model, 'dual_exchanger_supply_min' + str(t) + '_' + str(exchanger) + 'S' + str(k), dual_expr)
                    self.dual_exchanger_supply_max[exchanger, t, k] = Complementary_great(
                        cons_expr2, self.model, 'dual_exchanger_supply_max' + str(t) + '_' + str(exchanger) + 'S' + str(k), dual_expr)
                    self.dual_exchanger_return_min[exchanger, t, k] = Complementary_great(
                        cons_expr3, self.model, 'dual_exchanger_return_min' + str(t) + '_' + str(exchanger) + 'S' + str(k), dual_expr)
                    self.dual_exchanger_return_max[exchanger, t, k] = Complementary_great(
                        cons_expr4, self.model, 'dual_exchanger_return_max' + str(t) + '_' + str(exchanger) + 'S' + str(k), dual_expr)
    ...
    def build_gas_system_original_and_dual_constrains(self):
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_node_gas_balance[node, t, k] = Complementary_equal(
                        1*cons_expr1, self.model, 'dual_node_gas_balance_time_' + str(t) + '_node_' + str(node) + 'S_' + str(k), dual_expr)
            ...
            for t in range(0, T-1):
                for k in range(K):
                    ...
                    self.dual_linepack_with_pressure[line, t, k] = Complementary_equal(
                        cons_expr1, self.model, 'dual_gas_linepack_equation_line_' + str(line) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
                    self.dual_linepack_with_time[line, t, k] = Complementary_equal(
                        cons_expr2, self.model, 'dual_gas_linepack_with_time_line_' + str(line) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
        ...
        #             self.dual_compressor_consume[line, t, k], expr1 = Complementary_equal(cons_expr1, self.model, 'dual_gas_flow_active_line_' + str(line) + '_time_' + str(t) + '_S_' + str(k))
        ...
        #             self.dual_compressor_pressure_up[compressor, t, k], expr1 = Complementary_great(cons_expr1, self.model, 'dual_compressor_pressure_' + str(compressor) + '_t_' + str(t) + '_S_' + str(k))
        #             dual_expr.append(expr1)
        #
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_well_upper_output_min[well, t, k] = Complementary_great(
                        cons_expr1, self.model, 'dual_upper_well_output_min_' + str(well) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
                    self.dual_well_upper_output_max[well, t, k] = Complementary_great(
                        cons_expr2, self.model, 'dual_upper_well_output_max_' + str(well) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_well_lower_output_min[well, t, k] = Complementary_great(
                        cons_expr1, self.model, 'dual_lower_well_output_min_' + str(well) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
                    self.dual_well_lower_output_max[well, t, k] = Complementary_great(
                        cons_expr2, self.model, 'dual_lower_well_output_max_' + str(well) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
            ...
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_gas_node_pressure_min[node, t, k] = Complementary_great(
                        cons_expr1, self.model, 'dual_node_pressure_min_' + str(node) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
                    self.dual_gas_node_pressure_max[node, t, k] = Complementary_great(
                        cons_expr2, self.model, 'dual_node_pressure_max_' + str(node) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
        #
        # for line in range(self.gas_line_num):
        #     for t in range(T):
        #         for k in range(K):
        #             cons_expr1 = self.gas_flow_in[line, t, k] + self.gas_flow_out[line, t, k]
        #             self.dual_gas_flow_in_and_out_great_zero[line, t, k], expr1 = Complementary_great(cons_expr1, self.model, 'dual_gas_flow_great_zero_' + str(line) + '_t_' + str(t) + '_S_' + str(k))
        #             dual_expr.append(expr1)
        #
        for line in range(self.gas_line_num):
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_gas_flow_in_min[line, t, k]  = Complementary_great(
                        cons_expr1, self.model, 'dual_gas_flow_in_min'  + str(line) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
                    self.dual_gas_flow_in_max[line, t, k]  = Complementary_great(
                        cons_expr2, self.model, 'dual_gas_flow_in_max'  + str(line) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
                    self.dual_gas_flow_out_min[line, t, k] = Complementary_great(
                        cons_expr3, self.model, 'dual_gas_flow_out_min' + str(line) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
                    self.dual_gas_flow_out_max[line, t, k] = Complementary_great(
                        cons_expr4, self.model, 'dual_gas_flow_out_max' + str(line) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
        #
        for line in range(self.gas_line_num):
            for t in range(T):
                for k in range(K):
                    ...
                    self.dual_pccp_relax_great_zero[line, t, k] = Complementary_great(
                        cons_expr1, self.model, 'dual_pccp_relax_great_zero' + str(line) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
        ...
        #                 cons_expr1, self.model, 'weymouth_relax_left_auxiliary_' + str(line) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
        ...
        #                 self.model,
        #                 'weymouth_relax_left_soc_' + str(line) + '_t_' + str(t) + '_S_' + str(k), dual_expr)
        ...
        for k in range(K):
            ...
            for line in self.gas_inactive_line:
                for t in range(T):
                    ...
                    dual_vars1, constr1, expr1 = Complementary_equal_plus(cons_expr1, self.model, 'weymouth_relax_right_auxiliary1_' + str(line) + '_t_' + str(t) + '_S_' + str(k))
                    dual_vars2, constr2, expr2 = Complementary_equal_plus(cons_expr2, self.model, 'weymouth_relax_right_auxiliary2_' + str(line) + '_t_' + str(t) + '_S_' + str(k))
                    # 构建SOC 约束
    ...
    def utt(self):
        ...
        dual_equal = self.dual_expression_basic
        ## lower cost ##
        lower_cost = []
        for gen in range(self.generator_lower_num):
            for t in range(T):
                for k in range(K):
                    lower_cost.append(self.generator_lower_cost[gen] * self.lower_generator_power_output[gen, t, k])
        for chp in range(self.chp_lower_num):
            for t in range(T):
                for k in range(K):
                    lower_cost.append(self.chp_lower_coeff_p_1[chp] * self.lower_chp_power_output[chp, t, k])
                    lower_cost.append(self.chp_lower_coeff_h_1[chp] * self.lower_chp_heat_output[chp, t, k])
        for well in range(self.well_lower_num):
            for t in range(T):
                for k in range(K):
                    lower_cost.append(self.well_lower_cost[well] * self.lower_gas_well_output[well, t, k])
        lower_cost_k = sum(lower_cost)
        ## equivalnet
        obj_eq = []
        for gen in range(self.generator_upper_num):
            for t in range(T):
                for k in range(K):
                    obj_eq.append(-1 * self.dual_upper_generator_power_output_min[gen, t, k] * self.generator_upper_min[gen])
                    obj_eq.append(self.dual_upper_generator_power_output_max[gen, t, k] * self.generator_upper_max[gen])
        for well in range(self.well_upper_num):
            for t in range(T):
                for k in range(K):
                    obj_eq.append(-1 * self.dual_well_upper_output_min[well, t, k] * self.well_upper_output_min[well])
                    obj_eq.append(self.dual_well_upper_output_max[well, t, k] * self.well_upper_output_max[well])
        for chp in range(self.chp_upper_num):
            for t in range(T):
                for k in range(K):
                    temp = []
                    for point in range(self.chp_point_num):
                        temp.append(self.dual_upper_chp_point_less_one[chp, t, k, point])
                    obj_eq.append(sum(temp))
                    obj_eq.append(-1 * self.dual_upper_chp_point_sum_one[chp, t, k])
        obj_k = dual_equal - lower_cost + sum(obj_eq)
