...
class OneLayer:
    ...
    def build_heat_system(self):
        ...
        self.heat_pipe_start_tempe_supply       = \
            tonp( self.model.addVars(self.heat_pipe_num, T, K, name='heat_pipe_start_tempe_supply', lb=-1*INF, ub=INF))
        self.heat_pipe_end_tempe_supply         = \
            tonp( self.model.addVars(self.heat_pipe_num, T, K, name='heat_pipe_end_tempe_supply',   lb=-1*INF, ub=INF))
        self.heat_pipe_start_tempe_return       = \
            tonp( self.model.addVars(self.heat_pipe_num, T, K, name='heat_pipe_start_tempe_return', lb=-1*INF, ub=INF))
        self.heat_pipe_end_tempe_return         = \
            tonp( self.model.addVars(self.heat_pipe_num, T, K, name='heat_pipe_end_tempe_return',   lb=-1*INF, ub=INF))
    ...
    def build_lower_objective(self):
        ...
        for chp in range(self.chp_lower_num):
            for time in range(T):
                for k in range(K):
                    lower_objs.append(self.well_lower_cost[self.chp_lower_connection_well_index[chp]] * (self.lower_chp_heat_output[chp, time, k] * self.chp_lower_coeff_h_1[chp]) * 1)#self.upper_chp_gas_quoted_price[chp, time])
        # ！！！！？？？？？
